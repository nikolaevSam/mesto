...
let openPopup = () => {
  ...
  (buttonEdit.matches(':hover') === true) ?
    popup.classList.add('popup_opened')
    : false
};

let closePopup = (event) => {
  ((buttonClose.matches(':hover') === true) || (buttonSave.matches(':hover') === true)) ?
...
function formSubmitHandler(evt) {
  ...
  nameInput.value === "" || jobInput.value === "" ?
    alert('Please enter your name and job!') :
    (nameForm.textContent = nameInput.value, jobForm.textContent = jobInput.value)
}
